    ...
    <div class="mx-auto p-4">
        <form action="/rapport/edit_rapport/<?= $rapport['id'] ?>" method="post">
            ...
            <input type="text" name="nourriture_preconisee" id="nourriture_preconisee" 
                   value="<?= htmlspecialchars($rapport['nourriture_preconisee'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" <?= isset($_SESSION['role']) && $_SESSION['role'] === 'employe' ? 'disabled' : ''; ?>><br>

            <label for="nourriture_donnee">Nourriture donnée :</label>
            <input type="text" name="nourriture_donnee" id="nourriture_donnee" 
                   value="<?= htmlspecialchars($rapport['nourriture_donnee'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" <?= isset($_SESSION['role']) && $_SESSION['role'] === 'veterinaire' ? 'disabled' : ''; ?>><br>

            <label for="grammage_preconise">Grammage préconisé :</label>
            <input type="text" name="grammage_preconise" id="grammage_preconise" 
                   value="<?= htmlspecialchars($rapport['grammage_preconise'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" <?= isset($_SESSION['role']) && $_SESSION['role'] === 'employe' ? 'disabled' : ''; ?>><br>

            <label for="grammage_donne">Quantité donnée :</label>
            <input type="text" name="grammage_donne" id="grammage_donne" 
                   value="<?= htmlspecialchars($rapport['grammage_donne'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" <?= isset($_SESSION['role']) && $_SESSION['role'] === 'veterinaire' ? 'disabled' : ''; ?>><br>
